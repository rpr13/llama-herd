...
2. **Subprocess Orchestrator ([src/launcher.rs](file:///home/rpr/dev/llama-herd/src/launcher.rs))**: Responsible for constructing the precise command line arguments required by `llama-server` for both Single Preset and Router Modes. It manages target-specific process termination by tracking the subprocess PID and terminating only the exact running instance to prevent background port collisions without affecting other system processes.
