...
The `llama-server` executable path and the `models-dir` (where your models are located) MUST be defined here for the TUI dashboard to launch. If they are missing or invalid, LlamaHerd will launch an interactive **Setup Wizard** to help you configure them. Note: Running the CLI presets generation command (`llama-herd --ini`) does not require configuring `llama-server`; only `models-dir` must be defined.
